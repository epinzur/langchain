...
class CassandraGraphVectorStore(GraphVectorStore):
    ...
    def _get_metadata_filter(
        ...
        metadata_filter[_metadata_link_key(link=outgoing_link)] = _metadata_s_link_value()
    ...
    def _doc_to_node(self, doc: Document) -> Node:
        metadata = doc.metadata.copy()
        links = _deserialize_links(metadata.get(self.metadata_links_key))
        metadata[self.metadata_links_key] = links
    ...
    def add_nodes(
        ...
        **kwargs: Any,
        ...
        for node in nodes:
            ...
            combined_metadata[self.metadata_links_key] = _serialize_links(node.links)
        ...
        for node_id, text, text_embedding, metadata, incoming_links in tuples:
            ...
            if isinstance(metadata.get(self.metadata_links_key), set):
                metadata = metadata.copy()
                metadata[self.metadata_links_key] = list(metadata[self.metadata_links_key])

            attributes_blob =  MetadataMixin._serialize_md_dict(metadata)
    ...
    def get_node(self, id: str) -> Node | None:
        ...
        if doc is None:
            return None
        return self._doc_to_node(doc=doc)
    ...
    async def atraversal_search(
        ...
        **kwargs: Any,
        ...
        for id in visited_ids.keys():
            if id in retrieved_docs:
                yield self._restore_links(retrieved_docs[id])
            else:
                raise Exception(f"unexpected. retrieved_docs should contain id: {id}")
